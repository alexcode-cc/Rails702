...
# main vars
set :application, "rails701"
set :repo_url, "git@github.com:alexcode-cc/Rails701.git"
set :deploy_to, "/home/deploy7/rails701"
...
set :rvm_ruby_version, '3.2.1@rails7043'  # Defaults to: 'default'
...
namespace :rvm do
  ...
  desc "reset database with rvm"
  task :db_reset do
    on roles(fetch(:rvm_roles, :all)), wait: 10 do
      rails_env = fetch(:rails_env, 'production')
      rvm_path = fetch(:rvm_path, '~/.rvm')
      rvm_do_prefix = "#{rvm_path}/bin/rvm #{fetch(:rvm_ruby_version)} do"
      execute "cd #{current_path}; #{rvm_do_prefix} rails db:reset RAILS_ENV=#{rails_env}"
    end
  end
end
